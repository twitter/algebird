...
object VectorSpaceProperties extends Properties("ScalingOperator") with BaseVectorSpaceProperties {
...
trait BaseVectorSpaceProperties {
  def isEqualIfZero[F, C[_]](eqfn: (C[F], C[F]) => Boolean)(implicit vs: VectorSpace[F, C], arb: Arbitrary[C[F]]) = forAll { (a: C[F]) =>
    eqfn(VectorSpace.scale(vs.field.zero, a), vs.group.zero)
  }

  def distributesWithPlus[F, C[_]](eqfn: (C[F], C[F]) => Boolean)(implicit vs: VectorSpace[F, C], arbC: Arbitrary[C[F]], arbF: Arbitrary[F]) = forAll { (a: C[F], b: C[F], c: F) =>
    val v1 = VectorSpace.scale(c, vs.group.plus(a, b))
    val v2 = vs.group.plus(VectorSpace.scale(c, a), VectorSpace.scale(c, b))
    eqfn(v1, v2)
  }

  def isAssociative[F, C[_]](eqfn: (C[F], C[F]) => Boolean)(implicit vs: VectorSpace[F, C], arbC: Arbitrary[C[F]], arbF: Arbitrary[F]) = forAll { (a: C[F], b: F, c: F) =>
    val v1 = VectorSpace.scale(c, VectorSpace.scale(b, a))
    val v2 = VectorSpace.scale(vs.field.times(c, b), a)
    eqfn(v1, v2)
  }

  def identityOne[F, C[_]](eqfn: (C[F], C[F]) => Boolean)(implicit vs: VectorSpace[F, C], arb: Arbitrary[C[F]]) = forAll { (a: C[F]) =>
    eqfn(VectorSpace.scale(vs.field.one, a), a)
  }

  def distributesOverScalarPlus[F, C[_]](eqfn: (C[F], C[F]) => Boolean)(implicit vs: VectorSpace[F, C], arbC: Arbitrary[C[F]], arbF: Arbitrary[F]) = forAll { (a: C[F], b: F, c: F) =>
