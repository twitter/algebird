...
class SketchMapMonoid[K, V](val params: SketchMapParams[K])
  ...
  override def bufferedSumOption(items: Seq[SketchMap[K, V]]): Option[SketchMap[K, V]] =
    if(items.isEmpty) None
    else {
      val newValuesTable = Monoid.sum(items.map(_.valuesTable))
      val heavyHittersSet = Monoid.sum(items.map(_.heavyHitterKeys.toSet))
      val newtotalValue = Monoid.sum(items.map(_.totalValue))
      Some(SketchMap(
        newValuesTable,
  ...
  override def statefulSummer() = new BufferedSumAll(1000)(this)
