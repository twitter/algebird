...
/**
 * Preparer is a way to build up an Aggregator through composition using a more natural API: it allows you to
 * start with the input type and describe a series of transformations and aggregations from there, rather than
 * starting from the aggregation and composing "outwards" in both directions.
 *
 * Uses of Preparer will always start with a call to Preparer[A], and end with a call to monoidAggregate or a
 * related method, to produce an Aggregator instance.
 */
sealed trait Preparer[A, T] extends java.io.Serializable {
  ...
  /**
   * Produce a new Preparer that chains this one-to-many transformation. Because "many" could include "none",
   * this limits future aggregations to those done using monoids.
   */
  ...
  /**
   * Filter out values that do not meet the predicate. Like flatMap, this limits future aggregations to
   * MonoidAggregator.
   */
  ...
  /**
   * count and following methods all just call monoidAggregate with one of the standard Aggregators. see the
   * Aggregator object for more docs.
   */
  ...
  /**
   * transform a given Aggregator into a MonoidAggregator by lifting the reduce and present stages into Option
   * space
   */
  ...
  /**
   * headOption and following methods are all just calling lift with standard Aggregators see the Aggregator
   * object for more docs
   */
...
/**
 * A Preparer that has had zero or more map transformations applied, but no flatMaps. This can produce any
 * type of Aggregator.
 */
trait MapPreparer[A, T] extends Preparer[A, T] {
  ...
  /**
   * Split the processing into two parallel aggregations. You provide a function which produces two different
   * aggregators from this preparer, and it will return a single aggregator which does both aggregations in
   * parallel. (See also Aggregator's join method.)
   *
  ...
  /**
   * head and following methods all just call aggregate with one of the standard Aggregators. see the
   * Aggregator object for more docs.
   */
...
object MapPreparer {
   ...
   */
  def apply[A, T](fn: A => T): MapPreparer[A, T] = new MapPreparer[A, T] { val prepareFn: A => T = fn }

  /**
   * This is purely an optimization for the case of mapping by identity. It overrides the key methods to not
   * actually use the identity function.
   */
...
object FlatMapPreparer {
  ...
  /**
   * This is purely an optimization for the case of flatMapping by identity. It overrides the key methods to
   * not actually use the identity function.
   */
