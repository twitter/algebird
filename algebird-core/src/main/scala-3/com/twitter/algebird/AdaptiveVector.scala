...
object AdaptiveVector {
        ...
        (left, right) match {
          case (DenseVector(lv, ls, _), DenseVector(rv, _, _)) =>
            val vec = Semigroup.plus[IndexedSeq[V]](lv, rv) match {
              case v: Vector[?] => v.asInstanceOf[Vector[V]]
  ...
  implicit def semigroup[V: Semigroup]: Semigroup[AdaptiveVector[V]] =
    new AVSemigroup[V]
   ...
   */
  def denseEquiv[V: Equiv]: Equiv[AdaptiveVector[V]] =
