...
class TupleAggregatorsTest extends AnyWordSpec with Matchers {
  ...
  "GeneratedTupleAggregators" should {
    ...
    "Create an aggregator from a tuple of 2 aggregators" in {
      val agg: Aggregator[Int, Tuple2[Int, Int], Tuple2[Int, Int]] =
        Tuple2(MinAgg, MinAgg)
      assert(agg(data) == Tuple2(0, 0))
    }

    "Create an aggregator from a tuple of 3 aggregators" in {
      val agg: Aggregator[Int, Tuple3[Int, Int, Int], Tuple3[Int, Int, Int]] =
        Tuple3(MinAgg, MinAgg, MinAgg)
      assert(agg(data) == Tuple3(0, 0, 0))
    }

    "Create an aggregator from a tuple of 4 aggregators" in {
      val agg: Aggregator[Int, Tuple4[Int, Int, Int, Int], Tuple4[Int, Int, Int, Int]] =
        Tuple4(MinAgg, MinAgg, MinAgg, MinAgg)
      assert(agg(data) == Tuple4(0, 0, 0, 0))
    }

    "Create an aggregator from a tuple of 5 aggregators" in {
      val agg: Aggregator[Int, Tuple5[Int, Int, Int, Int, Int], Tuple5[Int, Int, Int, Int, Int]] =
        Tuple5(MinAgg, MinAgg, MinAgg, MinAgg, MinAgg)
      assert(agg(data) == Tuple5(0, 0, 0, 0, 0))
    }

    "Create an aggregator from a tuple of 6 aggregators" in {
      val agg: Aggregator[Int, Tuple6[Int, Int, Int, Int, Int, Int], Tuple6[Int, Int, Int, Int, Int, Int]] =
        Tuple6(MinAgg, MinAgg, MinAgg, MinAgg, MinAgg, MinAgg)
  ...
  "MultiAggregator" should {
    ...
    "Create an aggregator from a tuple of 2 aggregators" in {
      val agg: Aggregator[Int, Tuple2[Int, Int], Tuple2[Int, Int]] =
        MultiAggregator((MinAgg, MinAgg))
      assert(agg(data) == Tuple2(0, 0))
    }

    "Create an aggregator from a tuple of 3 aggregators" in {
      val agg: Aggregator[Int, Tuple3[Int, Int, Int], Tuple3[Int, Int, Int]] =
        MultiAggregator((MinAgg, MinAgg, MinAgg))
      assert(agg(data) == Tuple3(0, 0, 0))
    }

    "Create an aggregator from a tuple of 4 aggregators" in {
      val agg: Aggregator[Int, Tuple4[Int, Int, Int, Int], Tuple4[Int, Int, Int, Int]] =
        MultiAggregator((MinAgg, MinAgg, MinAgg, MinAgg))
      assert(agg(data) == Tuple4(0, 0, 0, 0))
    }

    "Create an aggregator from a tuple of 5 aggregators" in {
      val agg: Aggregator[Int, Tuple5[Int, Int, Int, Int, Int], Tuple5[Int, Int, Int, Int, Int]] =
        MultiAggregator((MinAgg, MinAgg, MinAgg, MinAgg, MinAgg))
      assert(agg(data) == Tuple5(0, 0, 0, 0, 0))
    }

    "Create an aggregator from a tuple of 6 aggregators" in {
      val agg: Aggregator[Int, Tuple6[Int, Int, Int, Int, Int, Int], Tuple6[Int, Int, Int, Int, Int, Int]] =
        MultiAggregator((MinAgg, MinAgg, MinAgg, MinAgg, MinAgg, MinAgg))
      assert(agg(data) == Tuple6(0, 0, 0, 0, 0, 0))
    }
    ...
    "Create a MonoidAggregator from a tuple of 2 MonoidAggregators" in {
      val agg: MonoidAggregator[Long, Tuple2[Long, Long], Tuple2[Long, Long]] =
        MultiAggregator((SizeAgg, SizeAgg))
      assert(agg(longData) == Tuple2(6, 6))
    }

    "Create a MonoidAggregator from a tuple of 3 MonoidAggregators" in {
      val agg: MonoidAggregator[Long, Tuple3[Long, Long, Long], Tuple3[Long, Long, Long]] =
        MultiAggregator((SizeAgg, SizeAgg, SizeAgg))
      assert(agg(longData) == Tuple3(6, 6, 6))
    }

    "Create a MonoidAggregator from a tuple of 4 MonoidAggregators" in {
      val agg: MonoidAggregator[Long, Tuple4[Long, Long, Long, Long], Tuple4[Long, Long, Long, Long]] =
        MultiAggregator((SizeAgg, SizeAgg, SizeAgg, SizeAgg))
  ...
  "MapAggregator" should {
    ...
    "Create an aggregator from 2 (key, aggregator) pairs" in {
      val agg: MapAggregator[Int, Tuple2[Int, Long], String, Long] =
        MapAggregator(("key1", MinLongAgg), ("key2", SizeAgg))
      val expectedMap = Map("key1" -> 0, "key2" -> 6)
      assert(agg(data) == expectedMap)
      assert(agg.keys == expectedMap.keys)
    }

    "Create an aggregator from 3 (key, aggregator) pairs" in {
      val agg: MapAggregator[Int, Tuple3[Int, Long, Int], String, Long] =
        MapAggregator(("key1", MinLongAgg), ("key2", SizeAgg), ("key3", MinLongAgg))
      val expectedMap = Map("key1" -> 0, "key2" -> 6, "key3" -> 0)
      assert(agg(data) == expectedMap)
      assert(agg.keys == expectedMap.keys)
    }

    "Create an aggregator from 4 (key, aggregator) pairs" in {
      val agg: MapAggregator[Int, Tuple4[Int, Long, Int, Long], String, Long] =
        MapAggregator(("key1", MinLongAgg), ("key2", SizeAgg), ("key3", MinLongAgg), ("key4", SizeAgg))
      val expectedMap = Map("key1" -> 0, "key2" -> 6, "key3" -> 0, "key4" -> 6)
      assert(agg(data) == expectedMap)
      assert(agg.keys == expectedMap.keys)
    }

    "Create an aggregator from 5 (key, aggregator) pairs" in {
      val agg: Aggregator[Int, Tuple5[Long, Int, Int, Int, Int], Map[String, Long]] =
        MapAggregator(
          ("key1", SizeAgg),
          ("key2", MinLongAgg),
          ("key3", MinLongAgg),
    ...
    "Create an aggregator from 6 (key, aggregator) pairs" in {
      val agg: Aggregator[Int, Tuple6[Long, Int, Int, Int, Int, Int], Map[String, Long]] =
        MapAggregator(
          ("key1", SizeAgg),
          ("key2", MinLongAgg),
          ("key3", MinLongAgg),
    ...
    "Create an aggregator from 7 (key, aggregator) pairs" in {
      val agg: Aggregator[Int, Tuple7[Long, Int, Int, Int, Int, Int, Int], Map[String, Long]] =
        MapAggregator(
          ("key1", SizeAgg),
          ("key2", MinLongAgg),
          ("key3", MinLongAgg),
    ...
    "Create an aggregator from 8 (key, aggregator) pairs" in {
      val agg: Aggregator[Int, Tuple8[Long, Int, Int, Int, Int, Int, Int, Int], Map[String, Long]] =
        MapAggregator(
          ("key1", SizeAgg),
          ("key2", MinLongAgg),
          ("key3", MinLongAgg),
    ...
    "Create an aggregator from 9 (key, aggregator) pairs" in {
      val agg: Aggregator[Int, Tuple9[Long, Int, Int, Int, Int, Int, Int, Int, Int], Map[String, Long]] =
        MapAggregator(
          ("key1", SizeAgg),
          ("key2", MinLongAgg),
          ("key3", MinLongAgg),
