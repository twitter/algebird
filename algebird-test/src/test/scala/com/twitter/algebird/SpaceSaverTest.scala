...
class SpaceSaverLaws extends CheckProperties {
  ...
  property("SpaceSaver can serialize/deserialize itself") {
    forAll { ss: SpaceSaver[String] =>
      ...
      SpaceSaver.fromBytes(ssAsBytes, SpaceSaverTest.arrayByteToString).get == ss
    }
  }
...
class SpaceSaverTest extends WordSpec with Matchers {
  ...
  "SpaceSaver" should {
    ...
    "Serialize should not change in time" in {
      ...
      val ssOneBytes = SpaceSaver.toBytes(ssOne, SpaceSaverTest.stringToArrayByte)

      val ssMany = SpaceSaver(40, "another string that is not empty", 100)
      val ssManyBytes = SpaceSaver.toBytes(ssMany, SpaceSaverTest.stringToArrayByte)

      ssOneBytes shouldEqual Array(1, 0, 0, 0, 40, 0, 0, 0, 24, 115, 116, 114, 105, 110, 103, 32, 116, 104, 97, 116, 32, 105, 115, 32, 110, 111, 116, 32, 101, 109, 112, 116, 121)
      ssManyBytes shouldEqual Array(2, 0, 0, 0, 40, 0, 0, 0, 1, 0, 0, 0, 32, 97, 110, 111, 116, 104, 101, 114, 32, 115, 116, 114, 105, 110, 103, 32, 116, 104, 97, 116, 32, 105, 115, 32, 110, 111, 116, 32, 101, 109, 112, 116, 121, 0, 0, 0, 0, 0, 0, 0, 100, 0, 0, 0, 0, 0, 0, 0, 0)
    }
  }
}
