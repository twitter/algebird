...
/**
 * Basic ChainableCallbackCollectorBuilder laws, useful for testing any monad.
 */

object ChainableCallbackCollectorBuilderLaws {
  import BaseProperties._

  def defaultEq[T] = { (t0: T, t1: T) => (t0 == t1) }

  def leftIdentity[M[_], T, U](eq: (M[U], M[U]) => Boolean = defaultEq[M[U]])(implicit monad: ChainableCallbackCollectorBuilder[M], arb: Arbitrary[T], arbfn: Arbitrary[(T) => M[U]]) =
    forAll { (t: T, fn: T => M[U]) => eq(t.pure[M].flatMap(fn), fn(t)) }

  def rightIdentity[M[_], T](eq: (M[T], M[T]) => Boolean = defaultEq[M[T]])(implicit monad: ChainableCallbackCollectorBuilder[M], arb: Arbitrary[M[T]]) =
    forAll { (mt: M[T]) => eq(mt.flatMap { _.pure[M] }, mt) }

  def associative[M[_], T, U, V](eq: (M[V], M[V]) => Boolean)(implicit monad: ChainableCallbackCollectorBuilder[M], arb: Arbitrary[M[T]], fn1: Arbitrary[(T) => M[U]],
  ...
  def monadLaws[M[_], T, U, R](eq: (M[R], M[R]) => Boolean = defaultEq[M[R]])(implicit monad: ChainableCallbackCollectorBuilder[M], arb: Arbitrary[M[T]], fn1: Arbitrary[(T) => M[U]],
